...
class DesignerNewsVotesRepository(
    ...
    fun upvoteStory(
        ...
        val response = withContext(contextProvider.io) { remoteDataSource.upvoteStory(storyId, userId) }
        onResult(response)
    }
    ...
    fun upvoteComment(
        ...
        // right now we just care whether the response is successful or not.
        // TODO save the response in the database
        val response = withContext(contextProvider.io) { remoteDataSource.upvoteComment(commentId, userId) }
        onResult(response)
    }
